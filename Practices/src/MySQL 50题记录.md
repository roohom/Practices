...
-- 17、按平均成绩从高到低显示所有学生的所有课程的成绩以及平均成绩
# 按平均成绩排序的学号和对应的平均成绩
select a.s_id,sum(a.s_score) / count(a.c_id) as avg
from score as a
group by a.s_id
order by sum(a.s_score) / count(a.c_id) desc
...
from score as q,
     (select a.s_id, sum(a.s_score) / count(a.c_id) as avg
      from score as a
      group by a.s_id
      order by sum(a.s_score) / count(a.c_id) desc) as p,
     course as c
where q.s_id = p.s_id
  and q.c_id = c.c_id
order by p.avg desc;
...
select a.s_id as 学号,sum(a.s_score) as 总成绩,
...
-- 方法二 参考了答案，将两个表联合，并将表二中比表一大的前两个个拿出来
select a.s_id, a.c_id, a.s_score
from score as a
